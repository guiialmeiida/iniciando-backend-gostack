...
let ListProviderDayAvailabilityService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const availability = eachHourArray.map(hour => {
      const hasApponintmentHour = appointments.find(appointment => (0, _dateFns.getHours)(appointment.date) === hour);
      const compareDate = new Date(year, month - 1, day, hour);
      return {
        hour,
        available: !hasApponintmentHour && (0, _dateFns.isAfter)(compareDate, currentDate)
      };
    });
    return availability;
  }
